...
public class regist extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (user_Name == "") {
			...
			try {
				if (method.email_check_back(method.DataConn(), user_email)) {
					String script = "<script>alert('该邮箱已注册！！！');location.href='regist.jsp'</script>";
					response.getWriter().println(script);
				} else if (!method.ifExist(method.DataConn(), user_Name)) {
					if (user_Password1.equals(user_Password2)) {
						if (email_code.equals(yanzhengma)) {
							...
							method.insertUserName(method.DataConn(),user_Name);
